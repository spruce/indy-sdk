...
impl<'a> Setup<'a> {
    ...
    */
        ...
        match pool_handle {
            Some(pool_handle) => {
                if config.num_trustees > 0 {
                    trustees = Some(Setup::create_trustees(
                        wallet,
                        pool_handle,
                        config.num_trustees,
                    ));
    ...
    fn setup_pool(connect_to_pool: bool) -> (String, Option<i32>) {
        indy::pool::set_protocol_version(PROTOCOL_VERSION as usize)
            .wait()
            .unwrap();
        ...
        if connect_to_pool {
            let pool_handle = indy::pool::open_pool_ledger(&pool_name, None)
                .wait()
                .unwrap();
...
impl<'a> Drop for Setup<'a> {
    fn drop(&mut self) {
        self.pool_handle
            .map(|handle| indy::pool::close_pool_ledger(handle).wait().unwrap());
        indy::pool::delete_pool_ledger(self.pool_name.as_str())
            .wait()
            .unwrap();
    }
}
...
impl Entities {
    ...
    */
    pub fn dids(&self) -> Vec<&str> {
        self.0.iter().map(|trust| trust.did.as_str()).collect()
    }
}
